...
class MemberInvoice extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        if($this->receipient != 'Member'){
            $subject = 'Invoice for - ' . $this->member_details->first_name . ' ' . $this->member_details->family->family_code;
        }

        return (new MailMessage)
        ->subject($subject)
        ->attach($this->file_path)
        ->view('notifications.new-member-invoice', [
